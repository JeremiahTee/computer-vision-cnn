...
for epoch in range(epochs):
    training_loss,training_accuracy = Train(model,optimizer,train_dl,device)
    test_loss , test_accuracy = Test(model,test_dl,device)
    training_loss.extend(training_loss)
    training_accuracy.extend(training_accuracy)
    test_loss_tracker.append(test_loss)
    test_accuracy_tracker.append(test_accuracy)
    print(f'epoch: {epoch}')
    print('\t training loss/accuracy: {0:.5f}/{1:.5f}'.format(sum(training_loss)/len(training_loss), sum(training_accuracy)/len((training_accuracy))))
    print('\t testing loss/accuracy: {0:.5f}/{1:.5f}'.format(test_loss, test_accuracy))